...
def battle(board_rows, board_cols, player, player_pos, *args):
    """
    The battle function is one of the main functions of the game. It uses the board class to handle
    all movement and creature position. Handles attacking by both the player and enemies, and also
    handles the turn-based system of battle. Battle will handle all errors related to the game
    (not so much Python errors), since it would be awkward to pass error-handling outside of this
    function and still maintain a seamless experience.

    :param board_rows: the number of rows in the board.
    :param board_cols: the number of columns in the board.
    :param player: the player object.
    :param player_pos: a tuple that is formatted as: (row, col)
...
def create_initiative_order(player, enemy_list):
    ...
    # Inserts a tuple with (player, initiative). Initiative is calculated with a d20 + player dex.
    initiative_list.append((player, randint(1,20) + player_stats["dex"]))
    
    # Adds each enemy to the initiative list as a tuple (enemy, initiative). Initiative is
    # calculated with a d20 + enemy dex.
    for e in enemy_list:
...
def get_attack_tiles(board):
    ...
    for tile in attack_list:
        if (0 <= tile[0] < rows) and (0 <= tile[1] < cols):
