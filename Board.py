...
class Board(object):
    """
    Board is a class to allow for easy visualization of the landscape from both a user and
    programmer perspective. The board performs many functions to abstract away the difficulty of
    determining seemingly trivial things such as whether or not one character can attack another or
    where a certain character can move.
        Board also supports three terrain types:
            1. clear (default)
            2. light
            3. heavy
        Based on the given type, terrain can either be auto-generated, or specified after
        initialization (using set_terrain()).
    """
    
    def __init__(self, rows, cols, terrain="clear"):
        """
        Initializes a board (a list of lists) using the inputs for rows and cols (columns).
        :param rows: [int] the number of rows for the board.
        :param cols: [int] the number of columns for the board.
        :param terrain: [str] the amount of randomly generated terrain for the board.
        """
        
        self._EMPTY_SPACE = "  "
        
        # A dictionary to standardize the terrain icons.
    ...
    def check_space(self, row, col):
        """
        Checks to make sure that there isn't something in the specified position. If there is
        something there, raises a SpaceError.
        """
