...
class Enemy(Creature):
    ...
    def __init__(self, name, num=0):
        """
        Initialize an object of the Enemy class. Name is used to find the creature in the creature
        database. Num is an optional argument used when there are more than one of the same
        creature.
        """
    ...
    def ai(self, board):
        """
        Controls the enemy object during battle. Takes the board used for the battle as its only
        parameter. This function determines the difficulty level of the enemy based on its
        intelligence modifier.

        The AI is grouped into three difficulties:
            Easy (int: 1-3)- The enemy will move as close to the player as possible and then attack.

            Medium (int: 4-7)- The enemy will take allies into account. Melee attackers will flank
            with allies, ranged attackers will attack from a distance. There is, however, a 35%
            chance that they will make a "bad" move. For example, a melee attacker has a 35% chance
            that it will not flank.

            Hard (int: 8-10)- The enemy will behave exactly like the medium enemy, however, there
            will only be a 10% chance that it will make a "bad" move. Additionally, it will take the
            terrain into account, so that it knows to take cover, for example.
        """
